...
class all_in_call_test(unittest.TestCase):
    ...
    def test_even_uneven_stacks_allin_call(self):
        game = create_game_environment(1200, 800)
        game.new_hand()
        game.current_hand.all_in(game.player_one)
        self.assertEqual(game.player_one.bet_size, 1200)
        game.current_hand.call(game.player_two, 800, game.player_one)
        self.assertEqual(game.player_one.bet_size, 800)
        self.assertEqual(game.player_one.bankroll, 400)
        self.assertEqual(game.player_two.bet_size, 800)
        self.assertEqual(game.player_two.bankroll, 0)
